...
// ViewModel for managing the state and logic of the documents screen.
...
sealed interface DocsScreenUIEvent {
    ...
    data class OnSaveChunkingSettings(
        val chunkSize: Int,
...
data class DocsScreenUIState(
    ...
    val strategy: String = "whitespace",
)
...
class DocsViewModel(
    ...
    private val chunkingSettings: ChunkingSettings,
) : ViewModel() {
    ...
    init {
        _docsScreenUIState.update {
            it.copy(
                chunkSize = chunkingSettings.getChunkSize(),
                chunkOverlap = chunkingSettings.getChunkOverlap(),
                strategy = chunkingSettings.getSplitStrategy()
            )
        }
        viewModelScope.launch {
            documentsDB.getAllDocuments().collect {
                _docsScreenUIState.update { state -> state.copy(documents = it) }
            }
        }
    }
    ...
    fun onEvent(event: DocsScreenUIEvent) {
        when (event) {
            ...
            is DocsScreenUIEvent.OnSaveChunkingSettings -> {
                chunkingSettings.saveSettings(event.chunkSize, event.chunkOverlap, event.strategy)
                _docsScreenUIState.update {
                    it.copy(
    ...
    private suspend fun addChunksFromInputStream(
        ...
        val sizeVal = chunkingSettings.getChunkSize()
        val overlapVal = chunkingSettings.getChunkOverlap()
        val strategyVal = chunkingSettings.getSplitStrategy()
        val chunks =
